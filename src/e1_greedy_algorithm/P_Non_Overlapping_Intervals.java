...
public class P_Non_Overlapping_Intervals {
    ...
    public static void main(String[] args) {
        ...
        P_Non_Overlapping_Intervals sol = new P_Non_Overlapping_Intervals();
        System.out.println(sol.eraseOverlapIntervals(interval));
    }
    public int eraseOverlapIntervals(int[][] intervals) {
        ...
        Comparator<int []> meetingComparator = (c1 , c2)-> {
        ...
        List<int[]> startMeet = new ArrayList<>();
        startMeet.add(intervals[0]);

        //Var to track when prev meeting is completing.
        int prevMeetiingEndTime = intervals[0][1];
        int n = intervals.length;

        int index = 1;
        while(index < n){
            //If prev meeting is already running , move to next meeting.
            if(intervals[index][0] < prevMeetiingEndTime) {
                index++;
                continue;
            }

            //Prev meeting is completed, next meeting can be started.
            startMeet.add(intervals[index]);
            prevMeetiingEndTime = intervals[index][1];
            index++;
        }
        // System.out.println(startMeet);
        return intervals.length-startMeet.size();
